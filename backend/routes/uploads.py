...
import tempfile
import zipfile
# import textract  # moved to lazy import
import io
import json
...
# from google.cloud import speech  # moved to optional import
...
def require_textract():
	try:
		...
		raise HTTPException(
			status_code=500,
			detail=(
				"textract is not installed. Install it with `pip install textract` or install all requirements. "
				"Alternatively, avoid file extraction endpoints if you don't need them."
			)
		)
		...
		try:
			with zipfile.ZipFile(io.BytesIO(file_data)) as z:
				for inner_filename in z.namelist():
					...
					try:
						extracted_bytes = require_textract().process(tmp_path)
		...
		if ext in audio_extensions:
			...
			try:
				if speech is not None:
					...
					logger.info(f"GOOGLE_SPEECH_NOT_AVAILABLE: {{'file': '{filename}'}} -> fallback to textract")
					try:
						extracted_bytes = require_textract().process(tmp_path)
